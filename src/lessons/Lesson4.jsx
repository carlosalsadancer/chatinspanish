...
// CHEAT SHEET L3
// ═══════════════════════════════════════════════════════════════
const CHEAT_SHEET_L3 = [
  { display: "parada",      phrase: { es: "¿Dónde está la parada del autobús ADO?",         en: "Where is the ADO bus stop?" } },
  { display: "autobús",     phrase: { es: "¿Cuál autobús va a Cancún?",                      en: "Which bus goes to Cancún?" } },
  { display: "boleto",      phrase: { es: "Aquí está mi boleto.",                             en: "Here is my ticket." } },
  { display: "asiento",     phrase: { es: "Disculpe, ese es mi asiento. Tengo el ocho.",     en: "Excuse me, that is my seat. I have number eight." } },
  { display: "taxi",        phrase: { es: "¿Dónde puedo tomar un taxi?",                     en: "Where can I get a taxi?" } },
  { display: "dirección",   phrase: { es: "Esta es la dirección de mi hotel.",               en: "This is the address of my hotel." } },
  { display: "precio",      phrase: { es: "¿Cuál es el precio por llevarme al hostal?",      en: "What is the price to take me to the hostel?" } },
  { display: "reservación", phrase: { es: "Buen día, tengo una reservación a nombre de...", en: "Good day, I have a reservation under the name of..." } },
];
...
// ═══════════════════════════════════════════════════════════════
const SECTION = {
  id: "friends", title: "Making Friends", subtitle: "Conociendo Gente",
  color: C.turquesa, colorL: C.turquesaL, colorD: C.turquesaD,
  skills: [
    "Introduce yourself in Spanish",
    "Ask someone's name and where they're from",
    "Suggest going out for tacos",
...
function FinalQuiz({ speak, onComplete }) {
  const situations = [
    ...
    { en: "You want to ask their name. What do you say, in Spanish?",                    correct: "¿Y tú, cómo te llamas?" },
    { en: "You want to say nice to meet you. What do you say, in Spanish?",              correct: "Mucho gusto conocerte." },
    { en: "You want to ask where they're from. What do you ask, in Spanish?",            correct: "¿De dónde eres?" },
    { en: "You want to suggest going for tacos. What do you say, in Spanish?",           correct: "¿Vamos por unos tacos?" },
    { en: "You want to order tacos al pastor. What do you say, in Spanish?",             correct: "Quiero probar unos tacos de pastor." },
    { en: "You want to order a cold beer. What do you say, in Spanish?",                 correct: "Una cerveza bien fría, por favor." },
  ];
    ...
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 24, padding: "16px 18px", background: C.turquesaL, borderRadius: 16, border: `1.5px solid ${C.turquesa}20` }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: C.turquesaD, fontWeight: 700 }}>Making Friends · Quiz</div>
        </div>
      </div>
...
function LessonComplete({ onNext }) {
  ...
  return (
    <div style={{ textAlign: "center", padding: "40px 0", animation: "fadeUp 0.4s ease" }}>
      <Confetti show={celebrate} message="Lesson 4 Complete!" />
      ...
      <h2 style={{ fontSize: "clamp(24px,6vw,32px)", fontWeight: 900, color: C.textH, letterSpacing: -1, marginBottom: 8 }}>Lesson 4 Complete!</h2>
      <p style={{ fontSize: 15, color: C.textS, fontWeight: 500, marginBottom: 24 }}>You just made your first Mexican friend — in Spanish!</p>
...
// ═══════════════════════════════════════════════════════════════
// MAIN LESSON 4 COMPONENT
// ═══════════════════════════════════════════════════════════════
export default function Lesson4({ onBack, initialSlide = 0, onSlideChange, onComplete, userId, lesson3Completed = false }) {
  const { speak } = useTTS();
  ...
  async function saveProgress(slideNum, completed = false) {
    ...
    await supabase.from("progress").upsert(
      { user_id: userId, lesson_number: 4, completed, slide: slideNum },
    ...
    <div style={{ background: "#fff", minHeight: "100vh", paddingBottom: 64 }}>
        ...
        {slide === 0 && (
          <div>
            <div style={{ display: "flex", gap: 14, alignItems: "center", justifyContent: "center", marginBottom: 24 }}>
              ...
              <div>
                <div style={{ fontSize: "clamp(22px,5vw,28px)", fontWeight: 900, color: C.textH, letterSpacing: -0.8, lineHeight: 1.1 }}>Making Friends!</div>
              </div>
            </div>
            <div style={{ background: C.grisS, border: `1.5px solid ${C.grisB}`, borderRadius: 16, padding: "22px 24px", marginBottom: 16, position: "relative", overflow: "hidden" }}>
              ...
              <p style={{ margin: 0, fontSize: "clamp(14px,2.5vw,16px)", color: C.textS, lineHeight: 1.85, fontStyle: "italic", fontWeight: 500 }}>
                It's 8:30 PM. You're at the hostel common area in <strong style={{ color: C.textH }}>Playa del Carmen</strong> — backpackers from everywhere, music playing, tacos on the table. Someone sits next to you and says "Hola." This is your moment. Time to make your first Mexican friend — <strong style={{ color: C.turquesa }}>all in Spanish.</strong>
              </p>
            </div>
      ...
      <div style={{ background: "rgba(255,255,255,0.97)", backdropFilter: "blur(12px)", borderTop: `1.5px solid ${C.grisB}`, padding: "12px 20px", position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 50, boxShadow: "0 -2px 12px rgba(0,0,0,0.05)" }}>
        <div style={{ maxWidth: 720, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <span style={{ fontSize: 13, fontWeight: 800, color: C.negro, letterSpacing: -0.2 }}>Basic - Lesson 4 of 10</span>
      ...
      {showCheatSheet && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 999, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }} onClick={() => setShowCheatSheet(false)}>
          <div style={{ background: "#fff", borderRadius: 20, padding: "28px 24px", maxWidth: 480, width: "100%", maxHeight: "85vh", overflowY: "auto" }} onClick={(e) => e.stopPropagation()}>
            <div style={{ fontSize: 18, fontWeight: 900, color: C.textH, marginBottom: 4 }}>Cheat Sheet - L3</div>
            <div style={{ fontSize: 13, color: C.textS, marginBottom: 20 }}>Getting Around · Holbox</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
              {CHEAT_SHEET_L3.map((word, i) => (
